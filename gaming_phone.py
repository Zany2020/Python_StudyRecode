...
class GamingPhone(CMobilePhone):
    ...
    def __init__(self, brand: str, model: str, length: float, width: float, height: float,
                 ...
                 _cooling_power: int, game_mode: bool = False) -> None:

        #  super()调用父类构造，执行父类全部self.xx = xx
        ...
        self._cooling_power = _cooling_power
    ...
    def play_game(self, game_name: str):
        ...
        consume = 15 if self._game_mode else 10
        self._battery.consume(consume)#这是局部变量,同名没问题
